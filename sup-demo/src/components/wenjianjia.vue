...
<template>
  <div class="demo">
   <x-header :right-options="{showMore: true}" @on-click-more="showMenus = true">我的文件夹</x-header>
  ...
  data () {
	...
	methods:{
		...
		onaddbiji(){
								...
								let self=this;
								let data ='http://47.103.17.177:80/note/create'+'?'+'fatherID'+'='+'1'
					...
					//  this.$router.push({path: '/wenjianjia'})
					 self.$router.go(0)
					  ...
					  alert(  "请注意文件夹名不能为空");
							...
							let self=this;
							let data ='http://47.103.17.177:80/notebook/create'+'?'+'fatherID'+'&'+'bookName'+'='+result1
							this.$axios.post(data)
				.then(function(response){
							  //  if(response.data.code ==0){
		...
		getwjj(){
			...
			let data ='http://47.103.17.177:80/notebook/list'+'?'+'fatherID'+'='+'1'
			this.$axios.get(data)
			.then(function(response){
						  //  if(response.data.code ==0){
...
<style >
.p1{width:39px;height: auto;max-height: 90px;max-width: 39px; min-width:39px}
</style>
